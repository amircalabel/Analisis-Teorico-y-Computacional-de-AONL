...
\begin{figure}[h]
	...
	\begin{minipage}{0.45\textwidth}
		\centering
		\includegraphics[width=\textwidth]{Grafica de la funcion obj1.png}
	...
	\begin{minipage}{0.45\textwidth}
		\centering
		\includegraphics[width=\textwidth]{Superficie3d.png}
		%\caption{Segunda imagen}
		\label{fig:img2}
	\end{minipage}
	\caption{Representación gráfica de la función $f(x,y)$ en el dominio $[-100,100] \times [-100,100]$.}
...
\newpage
 ...
 \begin{itemize}
 	...
 	\item La forma del gradiente hace compleja la deteccion simple del otro punto crítico en el dominio de estudio $[-100, 100]^2$
 \end{itemize}
...
El análisis comparativo de los 615 experimentos realizados revela diferencias significativas en el desempeño de ambos algoritmos. A continuación se presenta un resumen exhaustivo basado en las métricas objetivas recopiladas:
\newpage
...
\begin{table}[h]
	...
	\begin{tabular}{lcc}
		\toprule
		\textbf{Métrica} & \textbf{Gradiente Descendente} & \textbf{BFGS} \\
		\midrule
		Iteraciones promedio & 1,852 & 11 \\
		Tasa de convergencia exitosa & 78\% & 100\% \\
		Precisión promedio & $1.2 \times 10^{-14}$ & $2.1 \times 10^{-15}$ \\
		Tiempo ejecución (s) & 0.045 & 0.003 \\
		Sensibilidad a parámetros & Alta & Baja \\
		\bottomrule
	\end{tabular}
\end{table}
\newpage
\subsubsection{Análisis de Eficiencia por Learning Rate}

El Gradiente Descendente demostró alta sensibilidad a la tasa de aprendizaje, con desempeño óptimo en $\alpha = 0.2$:

\begin{itemize}
	\item \textbf{$\alpha = 0.001$}: Convergencia incompleta en 5000 iteraciones desde puntos distantes
	\item \textbf{$\alpha = 0.01$}: Convergencia lenta con precisión moderada ($\sim 10^{-2}$)
	\item \textbf{$\alpha = 0.05$}: 1,652 iteraciones promedio para convergencia completa
	\item \textbf{$\alpha = 0.1$}: 839 iteraciones promedio con alta precisión
	\item \textbf{$\alpha = 0.2$}: 430 iteraciones promedio, óptimo balance velocidad-estabilidad
\end{itemize}

\subsubsection{Comportamiento por Región del Dominio}

\begin{itemize}
	\item \textbf{Puntos cercanos al origen}: Ambos algoritmos convergen rápidamente al óptimo global $(0,0)$
	\item \textbf{Puntos distantes ($\|x\| > 50$)}: BFGS mantiene convergencia robusta (10-15 iteraciones), mientras que GD requiere ajuste fino del learning rate
	\item \textbf{Regiones asimétricas}: BFGS muestra consistencia independiente de la simetría del punto inicial
\end{itemize}

\subsubsection{Casos Destacados}

\textbf{Mejor caso Gradiente Descendente}:
\begin{itemize}
	\item Punto inicial: $[0, 0]$
	\item Learning rate: 0.001
	\item Iteraciones: 1
	\item Valor óptimo: $0.0000000000$
\end{itemize}

\textbf{Mejor caso BFGS}:
\begin{itemize}
	\item Punto inicial: $[0, 0]$ 
	\item Iteraciones: 0
	\item Tiempo: 0.0003 segundos
	\item Valor óptimo: $0.0000000000$
\end{itemize}

\subsubsection{Conclusiones Comparativas}

\begin{itemize}
	\item \textbf{BFGS} supera consistentemente a \textbf{Gradiente Descendente} en eficiencia computacional y robustez
	\item \textbf{GD} requiere selección cuidadosa del learning rate para evitar convergencia lenta o divergencia
	\item \textbf{BFGS} logra 100\% de tasa de convergencia vs. 78\% de GD
	\item La ventaja de BFGS es más pronunciada en puntos iniciales distantes del óptimo
	\item Ambos algoritmos alcanzan precisión numérica comparable cuando convergen exitosamente
\end{itemize}
\end{multicols}
\newpage
La superioridad de BFGS en este estudio se atribuye a su capacidad para aproximar información de segundo orden, aprovechando la convexidad global de la función identificada en el análisis teórico.
\newpage
\begin{multicols}{2}
...
\end{multicols}
	...
	\end{table}
	\begin{multicols}{2}
		Para complementar el análisis teórico inicial, se realizó una evaluación numérica exhaustiva de la matriz Hessiana en puntos estratégicos del dominio, seleccionados mediante un criterio sistemático que busca caracterizar globalmente la convexidad de la función $f(x,y)$:
		
		\begin{itemize}
			\item \textbf{Punto $(0,0)$}: Punto crítico identificado analíticamente como candidato a mínimo global, esencial para verificar las condiciones de optimalidad.
			
			\item \textbf{Puntos $(1,1)$ y $(-1,-1)$}: Representantes de las regiones cercanas al origen en el primer y tercer cuadrante, permitiendo analizar el comportamiento local y la simetría de la función.
			
			\item \textbf{Puntos $(10,10)$ y $(-10,-10)$}: Puntos distantes del origen que caracterizan el comportamiento asintótico de la función y permiten evaluar la convexidad en regiones extremas del dominio de estudio $[-100, 100] \times [-100, 100]$.
		\end{itemize}
			Esta selección estratégica abarca las regiones más relevantes del espacio de búsqueda, proporcionando una visión comprehensiva de las propiedades de convexidad a diferentes escalas y en distintas orientaciones del plano cartesiano.
